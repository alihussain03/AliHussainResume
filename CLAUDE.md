...
1. **Move page content into data files**: extract page content from the `.njk` templates into JSON/YAML under `src/_data/` so edits are plain-text value changes with no HTML involved. (Blog posts are already plain Markdown and can be edited via GitHub's web UI today.)
   - **Done for all main pages**: content lives in `src/_data/` — `home.json` (hero, the two path cards + their buttons, impact highlights, trust cert); `experience.json`, `skills.json`, `education.json` (About); `services.json` (the offering cards); `projects.json` (`highlights` + `cases`). The templates loop over them. Ampersands go in as plain `&` (Nunjucks auto-escapes). Empty `tech: []` on a role/case hides its pill row; add a new object to `projects.json`'s `cases` array to add a case study. In `home.json` a path button with `"cv": true` renders the CV download button (uses `site.cv`); otherwise it uses the button's `href`. `variant` maps to `btn-primary`/`btn-secondary`.
   - **Footer links** are data-driven via `site.footerLinks` in `src/_data/site.json` (a `{label, href}` list looped in `base.njk`). Add a footer link by appending one entry — no HTML.
   - **Still inline** (structural, not repeating copy): section headings/eyebrows, the Services availability blurb, and static layout text. These are page scaffolding rather than owner-editable content.
